...
def dashboard_filter_ajax(request):
    ...
    # 计算预算剩余（假设总预算不随筛选变化，这里仅作为示例）
    total_budget = sum(b.amount for b in Budget.objects.filter(user=request.user))
    budget_remaining = total_budget - total_expense

    # 假设余额是所有账户的余额，不受筛选条件影响；如果需要动态计算，按需求修改
    accounts = Account.objects.filter(user=request.user)
    total_balance = sum(acc.balance for acc in accounts)

    # 计算类别分布
    category_distribution = {}
    expense_txs = transactions.filter(transaction_type='expense')
    for tx in expense_txs:
        cat_name = tx.category.name if tx.category else "Others"
        category_distribution[cat_name] = category_distribution.get(cat_name, 0) + float(tx.amount)
    ...
    data = {
        ...
        "recent_transactions": [
            {
                "category": tx.category.name if tx.category else "Others",
                "account": tx.account.name if tx.account else "Unknown",
                "date": tx.date.strftime("%Y-%m-%d"),
        "amount": -float(tx.amount) if tx.transaction_type == "expense" else float(tx.amount),
            }
...
def transaction_create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            transaction.save()
            return redirect('finance_app:transactions')
    else:
...
def transactions_view(request):
    ...
    return render(request, "finance_app/transactions.html", context)
...
def analytics(request):
    ...
    # 1. Income and expenditure trend chart data: summarize income and expenditure by date
    ...
    # 2. Category share data (expenditure only)
    ...
    ## 3. Changes in balances of each account
    account_data = {}
    accounts = Account.objects.filter(user=user)
    ...
    context = {
        # 使用 json.dumps 将 Python 数据转换为合法 JSON 字符串
        "income_trend_dates": json.dumps(list(income_trend.keys())),
        "income_trend_values": json.dumps(list(income_trend.values())),
        "expense_trend_values": json.dumps(list(expense_trend.values())),
